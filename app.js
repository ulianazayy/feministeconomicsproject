  ...
  app.innerHTML = `
    <section class="view setup-grid">
      <div class="card">
        ...
        <p class="lead">
          Each person receives a neutral participant number. Visible profiles stay equivalent;
          hidden structural paths remain invisible until the final reveal.
        </p>
        <form class="field-stack" data-form="join">
          <label>
            Display name
            <input name="playerName" maxlength="24" placeholder="Your name" value="${escapeHtml(state.playerName)}" />
          </label>
          <label>
            Room code
            ...
            .map(
              (seatNumber, index) => `
                <div class="player-row">
                  <span class="avatar ${index % 2 ? "" : "alt"}" aria-hidden="true"></span>
                  <strong>${participantLabel(seatNumber)}</strong>
                  <span class="status-pill">Seat ${String(seatNumber).padStart(2, "0")}</span>
                </div>
              `,
            )
            .join("")}
